...
- On KOBO each form must have a service called "Backend" configured with endpoint to backendscript, JSON format and two headers ("country" that must have the value of country (example "it") and "label" that must have the value of standard label that form has to put to issue sending in reports repo (example "fake news").
